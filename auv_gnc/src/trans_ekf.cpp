...
void TransEKF::sixDofCB(const auv_msgs::SixDoF::ConstPtr &raw)
{
    ...
    quaternion_.w() = raw->pose.orientation.w;
    quaternion_.x() = raw->pose.orientation.x;
    quaternion_.y() = raw->pose.orientation.y;
    quaternion_.x() = raw->pose.orientation.z;
    ...
    else
    {
        ...
        if (sensorMask.sum() > 0)
        {
            ...
            filtered.pose.position.x = inertialState_(0);
            filtered.pose.position.y = inertialState_(1);
            filtered.pose.position.z = inertialState_(2);
            filtered.pose.orientation = raw->pose.orientation;

            filtered.velocity.linear.x = inertialState_(3);
            filtered.velocity.linear.y = inertialState_(4);
            filtered.velocity.linear.z = inertialState_(5);
            filtered.velocity.angular = raw->velocity.angular;

            filtered.linear_accel.x = inertialState_(6);
            filtered.linear_accel.y = inertialState_(7);
            filtered.linear_accel.z = inertialState_(8);
